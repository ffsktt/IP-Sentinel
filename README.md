...
- 全部端点不可用时自动回退系统 DNS，并在每日简报的「DNS 链路健康」板块标记降级次数。
- 「疑似劫持」只在上述降级发生时才检查并记录：DoH 端点可用时探测流量根本不经过系统解析器，此时形态 A 的劫持是环境的既定配置而非故障，逐会话上报只会淹没真正需要关注的降级信号。
...
- 四态区域态势：✅ 正常 / ⚠️ 漂移 / ❌ 送中 / 🚨 盲区
  - 区域裁决以 YouTube 双核（Premium / Music）为准，URL 跳转仅作副雷达。双核同时无响应记为**盲区**而非漂移——副雷达无法独立定区，否则一次探测中断会被计成一次区域漂移。任一核心命中 CN 时优先判**送中**，即使另外两核已哑火。
- 按网段分组的送中率与环比
- 异常地址：将漂移与送中合并展示，IPv4 按 `/24` 全量压缩为命中图与精确末段范围，IPv6 随机抽查最多 3 个地址
- 信用净化成功率
...
网段分组取自 `IP_POOL_FILTER`（按首次命中匹配）；未配置过滤时按 IPv4 `/24`、IPv6 `/48` 自动聚合。网段表按送中率降序排列，只渲染前 8 段，其余折叠为一行汇总——这是受 Telegram 单条消息 4096 字符上限约束的取舍。异常地址区将 `DRIFT` 与 `CN` 合并计算。IPv4 命中图固定为 16 格，每格对应一个 `/28`：`.` 表示无命中，`1`–`F` 表示命中 1–15 个地址，`#` 表示该格 16 个地址全部命中；下一行的“异常地址范围”列出上一行 `/24` 内全部异常地址的最后一段，`1-3` 即该网段的 `.1` 至 `.3`。IPv6 地址较长，仅随机抽查最多 3 个。

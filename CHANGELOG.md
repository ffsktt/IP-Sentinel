...
### 🐛 Bug Fixes
- **Bracketed IPv6 failed every source-pinning guard, so the whole v6 half of a pool egressed from one address** — `ip_pool.sh` writes IPv6 into the per-job config bracketed (`[2a12:…::10c0]`), but `sentinel_net_init` and the three inline fallbacks validated the *bracketed* string against a class admitting no `[`. Every IPv6 job fell through the guard, skipped `--interface` entirely and left the kernel to pick a source address — silently, because the "egress IP lost" warning lives in the other branch. IPv4 matched and was unaffected. Observed on a 876-IP node: all 256 v6 addresses reported `2a12:…::10ff` as their egress and drew 4695 `DRIFT` + 7491 `EGRESS_MISMATCH` events over three days, i.e. the v6 pool had never received any effective maintenance. All five guards now strip the armour first and validate the bare address
...
- **DNS hijack self-check fired once per session on every sniproxy-fronted node** — the system resolver is hijacked *by design* there, which is exactly what `PROBE_DOH_URLS` routes around, so the check reported a fact about a resolver no probe uses: 37492 events in five days on one node, burying the DoH-degradation signal it was meant to accompany. Now gated on DoH selection having failed, which is the only state where the hijacked resolver can actually reach probe traffic

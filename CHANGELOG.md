...
### 🐛 Bug Fixes
- **Bracketed IPv6 failed every source-pinning guard, so a pool's entire IPv6 share egressed from one address** — `ip_pool.sh` writes IPv6 into the per-job config bracketed (`[2001:db8::1]`), but `sentinel_net_init` and the three inline fallbacks validated the *bracketed* string against a class admitting no `[`. Every IPv6 job fell through the guard, skipped `--interface` entirely and left the kernel to pick a source address — silently, because the "egress IP lost" warning lives in the other branch. IPv4 matched and was unaffected, so the symptom was every IPv6 address reporting one and the same egress: `sentinel_check_egress` filed a mismatch per session, the geo verdict recorded against each address actually described that one exit, and the IPv6 part of the pool received no effective maintenance at all. All five guards now strip the armour first and validate the bare address
- **The egress-address liveness check accepted prefixes of other pool addresses** — `ip addr show | grep -Fq "$addr"` matches `198.51.100.21` against a line carrying only `198.51.100.216`, and a pool spanning a whole /24 is full of such pairs. A departed address therefore read as live, `--interface` failed to bind, and the round burned with no degradation warning anywhere. Now `grep -Fqw`: `-F` keeps an IPv4 dot from acting as a regex wildcard, `-w` keeps the prefix out
- **`^[0-9a-fA-F:\[\]\.]+$` matched nothing at all** — inside a POSIX bracket expression a backslash is an ordinary member, not an escape, so the `]` of `\]` closed the class early and the remainder (`\.]+$`) demanded a literal `].`. `mod_quality.sh`'s deep-probe path carried this "robust" pattern and had therefore never bound an egress address, for IPv4 either. The original `\.` variant was merely loose — it silently accepted `\` as an address character
- **DNS hijack self-check fired once per session wherever the resolver is redirected on purpose** — a node fronting a local interception proxy has a hijacked system resolver *by design*, which is exactly what `PROBE_DOH_URLS` routes around, so the check reported a fact about a resolver no probe uses. On a pool node that is one event per IP per round, which buried the DoH-degradation signal it is meant to accompany. Now gated on DoH selection having failed, the only state where the hijacked resolver can actually reach probe traffic
- **A YouTube dual-core outage was filed as `DRIFT`** — region arbitration weights YT Premium and YT Music exclusively, so both probes returning empty guaranteed `YT_MATCH=0` and a drift verdict whenever the jump radar alone answered. Now yields `BLIND`; a `CN` hit from any single core still outranks the outage
...
- **DoH endpoint selection is cached per address family** — `sentinel_net_init` runs once per session and a pool node opens thousands of sessions a day, so the old code spent one `example.com` round trip per session (and up to 4x6s whenever the leading endpoints were down). `PROBE_DOH_TTL` (default 1800s, `0` disables) collapses that to a couple of probes per hour; a `PROBE_DOH_URLS` edit invalidates the cache
...
- **`--compressed` on HTML probes** — added to `mod_google.sh` roaming/YouTube requests and the `mod_quality.sh` fast-sonar probes. Outbound traffic drops ~40% versus the previous baseline even with throughput doubled
...
- Log rotation in `core/updater.sh` switched from a fixed 2000-line truncation to 50 MiB size-based rotation; a pool node produces tens of thousands of log lines per day, so the old threshold retained less than one round
